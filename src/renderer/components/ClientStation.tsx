...
export default function ClientStation({
  ...
  const [username, setUsername] = useState(() => localStorage.getItem('remember_player_username') || '');
  const [password, setPassword] = useState('');
  const [rememberMe, setRememberMe] = useState(() => localStorage.getItem('remember_player_checked') === 'true');
  ...
  const handleMemberLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const playerObj = data.user;

      if (playerObj.status === 'Suspended') {
        setLoginError('This member account is suspended. Contact cashier.');
        return;
      }

      if (playerObj.balance <= 0) {
      ...
      // Remember username
      if (rememberMe) {
        localStorage.setItem('remember_player_username', username.trim());
        localStorage.setItem('remember_player_checked', 'true');
      } else {
        localStorage.removeItem('remember_player_username');
        localStorage.setItem('remember_player_checked', 'false');
      }
      ...
      const updated = computers.map(pc => {
        if (pc.id === activePC.id) {
          return {
            ...
            currentUser: playerObj.username,
      ...
      onTriggerSocketEvent(activePC.id, 'unlock', `Player @${playerObj.username} successfully logged in at ${activePC.id} using loyalty balances.`);
      
      // Store current session id
      localStorage.setItem(`session_${activePC.id}`, data.session.id);

      // Trigger the player:login socket event too
      onTriggerSocketEvent(activePC.id, 'player:login' as any, JSON.stringify({ username: playerObj.username }));
  ...
  // Guest Quick Unlock
  const handleGuestQuickUnlock = async () => {
    try {
      const response = await fetch('/api/auth/guest/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pcId: activePC.id })
      });
      const data = await response.json();
      if (!data.success) {
        setLoginError(data.error || 'Guest login failed');
        return;
      }
      ...
      // Trigger the player:login socket event for guest
      onTriggerSocketEvent(activePC.id, 'player:login' as any, JSON.stringify({ username: data.user.username }));
    } catch (err: any) {
      setLoginError('Network error during guest login');
    }
  };
  ...
  const handleLogout = async () => {
    const sessionId = localStorage.getItem(`session_${activePC.id}`);
    if (sessionId) {
      try {
        ...
        localStorage.removeItem(`session_${activePC.id}`);
      } catch (err) {}
    }

    // Trigger the player:logout socket event
    onTriggerSocketEvent(activePC.id, 'player:logout' as any, `Player logged out`);
                ...
                {authTab === 'member' ? (
                  <form onSubmit={handleMemberLogin} className="space-y-3">
                    ...
                    <div className="flex items-center gap-2 py-1">
                      ...
                      />
                      <label htmlFor="remember-me" className="text-[11px] text-zinc-400 font-medium cursor-pointer select-none">Remember Username</label>
                    </div>
